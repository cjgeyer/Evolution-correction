...
Now that we have an analysis for one of the populations,
we are ready to carry out the same analyses for the other populations.
We need to analyze the data in the following files.
```{r more.data}
...
\begin{align*}
   ...
   \left[ \left( \frac{\partial \mu}{\partial b} \right)_{b = 0} \right]^2
   + 8 \mu^{-1} \nu
   \left( \frac{\partial \mu}{\partial b} \right)_{b = 0}
...
dfftns <- c(- 4 * nu.hat * dmu.dalpha * dmu.db^2 / mu.hat^2 +
    8 * nu.hat * dmu.db * d2mu.db.dalpha / mu.hat, 4 * dmu.db^2 / mu.hat, 0)
```
...
```{r dvaw}
dvaw <- c(8 * nu.hat * dmu.db * d2mu.db.dalpha, 4 * dmu.db^2, 0)
```
...
# Breeding Values Expressed in One Year versus Breeding Values Expressed in Another
...
Anna Peschel discovered the error in @kulbaba-et-al — that estimates
were not corrected for subsampling — which is corrected in this document.

Frank Shaw found a calculus error in the formulas for
$$
   \frac{\partial}{\partial \alpha}
   \frac{V_A(W)}{\mu}
$$
...
$$
   \frac{\partial V_A(W)}{\partial \alpha}
$$
in the first version of this correction uploaded to Zenodo
(https://zenodo.org/).
These have been fixed and the fixes reflected in the R code for
`dfftns` and `dvaw`.
